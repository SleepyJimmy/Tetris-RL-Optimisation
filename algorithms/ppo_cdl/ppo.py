...
MAX_ACTION_LENGTH = len(max(DATA, key=len))
SHAPE_SIZE = 8
...
class ForwardDynamicsModel(nn.Module): 
    """
        The forward dynamics model is used to compute the intrinsic reward.

        Essentially, the intrinsic reward should be higher when the prediction error is high, 
        indicating that the state being predicted has not been visted before 
    """
    ...
    def forward(self, state, action, shape, is_update=False):
        ...
        if is_update:
            ...
            output = next_state_pred.view(batch_size, 10, 20)
        else:
            output = next_state_pred.view(10, 20)
        return output
...
class ActorCritic(nn.Module):
    def __init__(self, state_dim):
        ...
        # actor
        """
            The actor takes the current and next state features because it needs to take into account all possible next state placements.
            Hence, we include all next states as input.
        """
