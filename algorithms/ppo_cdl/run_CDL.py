import torch
from CDL import CuriosityDrivenPPO
...
"""
    References: https://github.com/openai/large-scale-curiosity
"""
...
for run in range(1, no_of_runs + 1):
    ...
    agent = CuriosityDrivenPPO(obs_size, lr)
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        for t in range(max_timesteps):
            ...
            if timestep % update_timestep == 0:
                # print("Updating")
                ppo.update(memory)
                agent.update_dynamics_model(memory.board_state, memory.actions, memory.shapes, memory.next_states)
                memory.clear_memory()
                timestep = 0
