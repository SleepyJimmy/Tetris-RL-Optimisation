...
def get_features(obs, lines_cleared, rows_cleared, piece_coords, env):
    ...
    # for Tetris clears
    tetris_clear = check_4_line_clear(obs)
    right_col_holes = holes_in_right_col(obs)
    ...
    # For tetris clear 28 features
    feature_vector = np.array([
        landing_height, eroded_piece_cells, row_transitions, col_transitions, holes, 
        wells_sum, *peaks, *bumpiness, highest_col, tetris_clear, right_col_holes
    ])

    # # For tetris clear Dellacherie features
    # feature_vector = np.array([
    #     landing_height, eroded_piece_cells, row_transitions, col_transitions, holes, 
    #     wells_sum, tetris_clear, right_col_holes
    # ])
...
def check_4_line_clear(obs):
    """state feature to track if a move would generate a 4-line clear"""
    rows = []

    for i in range(4):
        rows.append(obs[obs.shape[0]-1-i, :])
...
def holes_in_right_col(obs):
    """state feature to track the amount of holes in the extreme right column"""
    col = obs[:, obs.shape[1] - 1]
    has_hole = any(col[i] == 0 and col[i-1] == 1 for i in range(1, len(col)))
    if not has_hole:
        return 0
    ...
    for i in range(j + 1):
        if col[i] == 1:
            ...
            while i < j:
                ...
                j -= 1
            break
    return len(col) - highest - counter