...
"""
    References: https://github.com/chengliu-LR/PPO
"""
...
class ActorCritic(nn.Module):
    ...
    def act(self, state, memory, act_pairs):
        ...
        act_idx = int(np.random.choice(len(action_probs), 1, p=action_probs.cpu().detach().numpy()))
        ...
        if act.item() in ACTION_LIST:
            ...
            raise ValueError("Action not found in ACTION_LIST") # can prolly remove this as this was for debugging 
...
class PPO():
    ...
    def update(self, memory):   
        ...
        # rewards = (rewards - rewards.mean()) / (rewards.std() + 1e-5)  #"""maybe can try removing normalisation?"""
